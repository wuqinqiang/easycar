...
func NewTM(tmServer string) *TM {
	return &TM{serverAddress: tmServer}
}

// Start start for a transaction
func (r *TM) Start(tran TransactionInterface) (gId string, err error) {
	if r.serverAddress == "" {
...
func (r *TM) RegisterBranch(ctx context.Context, branchList []common.BranchData) error {
	if len(branchList) == 0 {
		return fmt.Errorf("")
	}
	var (
...
func (r *TM) Commit(ctx context.Context) error {
	var (
		req  common.BranchData
		resp common.RespBase
	)
	req.SetGId(r.gId)
...
func (r *TM) Fail(ctx context.Context) error {
	var (
		req  common.BranchData
		resp common.RespBase
	)
